...
class StatsCogs(commands.Cog, name="stats"):
    ...
    async def thread_created(self, thread):
        ...
        try:
            ...
            await self.bot.pg_con.execute(
                ...
                "VALUES ($1,$2,$3,$4,$5,$6)",
                "threads", "THREAD_CREATED", thread.name, thread.name, datetime.now().replace(tzinfo=None), thread.id)
    ...
    async def thread_deleted(self, thread):
        ...
        await self.bot.pg_con.execute(
            "INSERT INTO updates(updated_table, action, before, after, date, primary_key) "
            "VALUES ($1,$2,$3,$4,$5,$6)",
            "threads", "THREAD_DELETED", thread.name, thread.name, datetime.now().replace(tzinfo=None), thread.id)
    ...
    async def thread_update(self, before, after):
        if before.slowmode_delay != after.slowmode_delay:
            ...
            await self.bot.pg_con.execute(
                "INSERT INTO updates(updated_table, action, before, after, date, primary_key) "
                "VALUES ($1,$2,$3,$4,$5,$6)",
                "threads", "THREAD_SLOWMODE_UPDATED", before.slowmode_delay, after.slowmode_delay, datetime.now().replace(tzinfo=None), after.id)
        if before.archived != after.archived:
            ...
            await self.bot.pg_con.execute(
                "INSERT INTO updates(updated_table, action, before, after, date, primary_key) "
                "VALUES ($1,$2,$3,$4,$5,$6)",
                "threads", "THREAD_ARCHIVED_UPDATED", before.archived, after.archived, datetime.now().replace(tzinfo=None), after.id)
        if before.locked != after.locked:
            ...
            await self.bot.pg_con.execute(
                "INSERT INTO updates(updated_table, action, before, after, date, primary_key) "
                "VALUES ($1,$2,$3,$4,$5,$6)",
                "threads", "THREAD_LOCKED_UPDATED", before.locked, after.locked, datetime.now().replace(tzinfo=None), after.id)
        if before.auto_archive_duration != after.auto_archive_duration:
            ...
            await self.bot.pg_con.execute(
                "INSERT INTO updates(updated_table, action, before, after, date, primary_key) "
                "VALUES ($1,$2,$3,$4,$5,$6)",
                "threads", "THREAD_AUTO_ARCHIVE_DURATION_UPDATED", before.auto_archive_duration, after.auto_archive_duration, datetime.now().replace(tzinfo=None), after.id)
        if before.is_private() != after.is_private():
            ...
            await self.bot.pg_con.execute(
                "INSERT INTO updates(updated_table, action, before, after, date, primary_key) "
                "VALUES ($1,$2,$3,$4,$5,$6)",
                "threads", "THREAD_PRIVATE_UPDATED", before.is_private(), after.is_private(), datetime.now().replace(tzinfo=None), after.id)
        if before.name != after.name:
            ...
            await self.bot.pg_con.execute(
                "INSERT INTO updates(updated_table, action, before, after, date, primary_key) "
                "VALUES ($1,$2,$3,$4,$5,$6)",
                "threads", "THREAD_NAME_UPDATED", before.name, after.name, datetime.now().replace(tzinfo=None), after.id)
